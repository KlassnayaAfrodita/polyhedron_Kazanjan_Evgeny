...
class Edge:
    ...
    def projection_lenght(self):
        lenght = sqrt((self.beg.x - self.fin.x) ** 2 +
                      (self.beg.y - self.fin.y) ** 2 +
                      (self.beg.z - self.fin.z) ** 2)
        return lenght * cos(abs(self.beg.z-self.fin.z) /
                            lenght) if lenght != 0 else 0
...
class Polyedr:
    ...
    def projection_lenght(self, x1, y1, z1, x2, y2, z2):
        lenght = sqrt((x1 - x2)**2 + (y1 - y2)**2 + (z1 - z2)**2)
        return lenght * cos(abs(z1-z2) /
                            lenght) if lenght != 0 else 0

    def sum_projection(self):
        sum_length = 0
        for edge in self.edges:
            x1, y1, z1, x2, y2, z2 = edge.beg.x, edge.beg.y, \
                edge.beg.z, edge.fin.x, edge.fin.y, edge.fin.z
            mid_x, mid_y = (x1+x2)/2, (y1+y2)/2
            if (not self.is_good_point(x1, y1) or
                    not self.is_good_point(x2, y2)) and \
                    (not self.is_good_point(mid_x, mid_y)):
                sum_length += self.projection_lenght(x1, y1, z1, x2, y2, z2)
        return sum_length/2
